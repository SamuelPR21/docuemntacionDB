...
SELECT
    ...
    p.id_patologia AS id_patologias,
    p.nombre,
    p.descripcion,
    p.es_activo,
    p.nombre_tecnico,
...
JOIN modulo9.patologias AS p
    ON p.id_patologia = ep.id_patologia;
...
FROM modulo9.patologias AS p
LEFT JOIN modulo9.especies_patologias AS ep
    ON ep.id_patologia = p.id_patologia
LEFT JOIN modulo4.patologias_signos AS ps
    ON ps.id_patologia = p.id_patologia
LEFT JOIN modulo4.predicciones AS pr
    ON pr.id_patologia = p.id_patologia
LEFT JOIN modulo4.alertas_patologicas AS ap
    ON ap.id_patologia = p.id_patologia
GROUP BY
    p.id_patologia,
    p.nombre;
...
LEFT JOIN (
    ...
    FROM modulo9.dispositivos_iot AS d0
    LEFT JOIN modulo9.sensores AS s
        ON s.id_dispositivo_iot = d0.id_dispositivo_iot
    LEFT JOIN modulo9.sensores_areas_asociadas AS saa
        ON saa.id_sensor = s.id_sensores
       AND (saa.fecha_finalizacion IS NULL OR saa.fecha_finalizacion > now())
    LEFT JOIN modulo2.activos_biologicos AS ab
        ON ab.id_dispositivo_iot = d0.id_dispositivo_iot
    ORDER BY d0.id_dispositivo_iot, saa.fecha_asociacion DESC NULLS LAST, saa.id_sensores_area_asociada DESC NULLS LAST
) AS d
