...
-- Esquema: modulo9
-- Motor: PostgreSQL
-- Versión: 1.2 (Segunda corrección)
-- =============================================================================
-- ÍNDICE
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_especies_nombre_unique_ci()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE EXCEPTION 'DUPLICATE_SPECIES: Ya existe una especie con el nombre "%" (validación case-insensitive). Use un nombre diferente.',
            NEW.nombre
            USING ERRCODE = 'P0101';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_especies_nombre_unique_ci();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_especies_nombre_formato()
RETURNS TRIGGER AS $$
DECLARE
    v_longitud INTEGER;
BEGIN
    ...
    IF v_longitud < 3 OR v_longitud > 50 THEN
        RAISE EXCEPTION 'INVALID_FORMAT: El nombre de la especie debe tener entre 3 y 50 caracteres. Longitud recibida: %.',
            v_longitud
            USING ERRCODE = 'P0102';
    END IF;
...
BEFORE INSERT OR UPDATE OF nombre ON modulo9.especies
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_especies_nombre_formato();
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_especies_audit();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_especies_no_delete()
RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'NO_PHYSICAL_DELETE: Las especies no pueden eliminarse físicamente. Use la desactivación lógica (es_activo = false). Especie afectada: "%".',
        OLD.nombre
        USING ERRCODE = 'P0103';
    RETURN NULL;
END;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_especies_no_delete();


-- =============================================================================
-- TRG-M9-05 — Unicidad de nombre de ciclo biológico (etapa) por especie,
--             case-insensitive
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_ciclos_biologicos_nombre_unique_ci();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_ciclos_biologicos_duracion_valida()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.duracion_dias IS NULL OR NEW.duracion_dias <= 0 THEN
        RAISE EXCEPTION 'INVALID_DURATION: La duración de la etapa debe ser un número entero positivo mayor a 0. Valor recibido: %.',
            NEW.duracion_dias
            USING ERRCODE = 'P0105';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_ciclos_biologicos_duracion_valida();


-- =============================================================================
-- TRG-M9-07 — Bloqueo de desactivación de etapa con activos biológicos
-- Tabla:    modulo9.ciclos_biologicos
...
BEGIN
    IF OLD.es_activo = TRUE AND NEW.es_activo = FALSE THEN
        ...
        -- Verificación 1: ciclos productivos que referencian esta etapa
        SELECT COUNT(*) INTO v_ciclos
        FROM modulo9.ciclos_productivos_biologicos
        WHERE id_ciclo_biologico = OLD.id_ciclo_biologico;

        -- Verificación 2: animales o lotes concretos activos cursando esta etapa
        -- (la validación que el trigger original omitía)
        ...
        IF v_ciclos > 0 OR v_activos > 0 THEN
            RAISE EXCEPTION
                ...
                '% ciclo(s) productivo(s) y % activo(s) biológico(s) (animales/lotes) '
                'actualmente en esta fase. Traslade los activos antes de proceder.',
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_ciclos_biologicos_no_desactivar_en_uso();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_patologias_nombre_unique_ci()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE EXCEPTION 'DUPLICATE_PATHOLOGY: Ya existe una patología con el nombre "%" en el catálogo global (case-insensitive).',
            NEW.nombre
            USING ERRCODE = 'P0107';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_patologias_nombre_unique_ci();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_patologias_no_delete()
RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'NO_PHYSICAL_DELETE: Las patologías no pueden eliminarse físicamente. Use la desactivación lógica (es_activo = false). Patología: "%".',
        OLD.nombre
        USING ERRCODE = 'P0108';
    RETURN NULL;
END;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_patologias_no_delete();
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_metricas_produccion_nombre_unique_ci();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_umbral_ambiental_duplicado()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_umbral_ambiental_duplicado();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_umbral_ambiental_rango_valido()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    IF NEW.valor_min >= NEW.valor_max THEN
        RAISE EXCEPTION 'INVALID_RANGE: El valor mínimo (%) no puede ser mayor o igual al valor máximo (%) para el umbral ambiental.',
            NEW.valor_min, NEW.valor_max
            USING ERRCODE = 'P0111';
    END IF;

    -- Validar que la especie esté activa
    SELECT es_activo INTO v_especie_activa
    FROM modulo9.especies
    WHERE id_especie = NEW.id_especie;

    IF v_especie_activa IS NULL OR v_especie_activa = FALSE THEN
        RAISE EXCEPTION 'INACTIVE_SPECIES: No se puede configurar un umbral para la especie ID % porque está inactiva o no existe.',
            NEW.id_especie
            USING ERRCODE = 'P0112';
    END IF;
...
BEFORE INSERT OR UPDATE ON modulo9.umbrales_ambientales
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_umbral_ambiental_rango_valido();
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_niveles_alerta_solapamiento();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_configuracion_global_unicidad()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_configuracion_global_heartbeat_valido()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.frecuencia_muestreo IS NULL OR NEW.frecuencia_muestreo <= 0 THEN
        RAISE EXCEPTION 'INVALID_CONFIG: La frecuencia de muestreo debe ser un entero positivo mayor a 0. Valor recibido: %.',
            NEW.frecuencia_muestreo
            USING ERRCODE = 'P0116';
    END IF;

    IF NEW.heartbeat IS NULL OR NEW.heartbeat <= 0 THEN
        RAISE EXCEPTION 'INVALID_CONFIG: El heartbeat debe ser un entero positivo mayor a 0. Valor recibido: %.',
            NEW.heartbeat
            USING ERRCODE = 'P0116';
    END IF;

    IF NEW.heartbeat < NEW.frecuencia_muestreo THEN
        RAISE EXCEPTION 'INVALID_CONFIG: El heartbeat (%) debe ser mayor o igual a la frecuencia de muestreo (%). No puede esperarse una señal antes del intervalo de envío.',
            NEW.heartbeat, NEW.frecuencia_muestreo
            USING ERRCODE = 'P0117';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_configuracion_global_heartbeat_valido();
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_finca_nombre_unique();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_finca_coordenadas_validas()
RETURNS TRIGGER AS $$
DECLARE
    v_lat NUMERIC;
    v_lng NUMERIC;
BEGIN
    ...
    IF NEW.tamano_h IS NULL OR NEW.tamano_h <= 0 THEN
        RAISE EXCEPTION 'INVALID_SIZE: El tamaño de la finca debe ser un valor numérico positivo mayor a cero. Valor recibido: %.',
            NEW.tamano_h
            USING ERRCODE = 'P0121';
    END IF;
    ...
    IF NEW.ubicacion IS NOT NULL THEN
        ...
        IF v_lat IS NOT NULL AND (v_lat < -90 OR v_lat > 90) THEN
            RAISE EXCEPTION 'INVALID_COORDINATES: La latitud debe estar entre -90 y 90. Valor recibido: %.',
                v_lat
                USING ERRCODE = 'P0122';
        END IF;

        IF v_lng IS NOT NULL AND (v_lng < -180 OR v_lng > 180) THEN
            RAISE EXCEPTION 'INVALID_COORDINATES: La longitud debe estar entre -180 y 180. Valor recibido: %.',
                v_lng
                USING ERRCODE = 'P0122';
        END IF;
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_finca_coordenadas_validas();
...
BEGIN
    -- Verificar infraestructuras directamente asociadas a la finca
    SELECT COUNT(*) INTO v_infra_count
    FROM modulo9.infraestructuras
    WHERE id_finca = OLD.id_finca;

    -- Verificar dispositivos IoT a través de infraestructuras (no hay FK directa a finca)
    SELECT COUNT(*) INTO v_iot_count
    FROM modulo9.dispositivos_iot d
    JOIN modulo9.infraestructuras i ON d.id_infraestructura = i.id_infraestructura
    WHERE i.id_finca = OLD.id_finca;

    -- Verificar activos biológicos a través de infraestructuras
    SELECT COUNT(*) INTO v_activos_count
    FROM modulo9.activos_biologicos ab
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_finca_no_delete_con_dependencias();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_infraestructura_nombre_unique_ci()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE EXCEPTION 'DUPLICATE_AREA: Ya existe un área productiva con el nombre "%" en esta finca (case-insensitive).',
            NEW.nombre
            USING ERRCODE = 'P0125';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_infraestructura_nombre_unique_ci();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_infraestructura_superficie_valida()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.superficie IS NULL OR NEW.superficie <= 0 THEN
        RAISE EXCEPTION 'INVALID_SURFACE: La superficie del área productiva debe ser un valor positivo mayor a cero. Valor recibido: %.',
            NEW.superficie
            USING ERRCODE = 'P0126';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_infraestructura_superficie_valida();


-- =============================================================================
-- TRG-M9-21 — Bloqueo de desactivación de infraestructura con dependencias
-- Tabla:    modulo9.infraestructuras
-- Evento:   BEFORE UPDATE OF es_activo
-- =============================================================================

CREATE OR REPLACE FUNCTION modulo9.trg_fn_infraestructura_no_desactivar_en_uso()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    IF OLD.es_activo = TRUE AND NEW.es_activo = FALSE THEN
        ...
        WHERE id_infraestructura = OLD.id_infraestructura
          AND es_activo          = TRUE;

        SELECT COUNT(*) INTO v_sensores
        FROM modulo9.sensores_areas_asociadas
        WHERE id_infraestructura = OLD.id_infraestructura
          AND tiene_estado       = TRUE;

        IF v_dispositivos > 0 OR v_sensores > 0 THEN
            RAISE EXCEPTION 'AREA_IN_USE: El área "%" tiene % dispositivo(s) activo(s) y % sensor(es) asociado(s). Desvincule los recursos antes de desactivar la infraestructura.',
                OLD.nombre, v_dispositivos, v_sensores
                USING ERRCODE = 'P0127';
        END IF;
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_infraestructura_no_desactivar_en_uso();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_dispositivo_serial_unique()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO v_count
    FROM modulo9.dispositivos_iot
    WHERE TRIM(serial) = TRIM(NEW.serial);

    IF v_count > 0 THEN
        RAISE EXCEPTION 'DUPLICATE_SERIAL: El número de serie "%" ya está registrado en el sistema. Cada dispositivo físico debe tener un único registro.',
            NEW.serial
            USING ERRCODE = 'P0128';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_dispositivo_serial_unique();


-- =============================================================================
-- TRG-M9-23 — Bloqueo de eliminación física de dispositivos IoT
-- Tabla:    modulo9.dispositivos_iot
-- Evento:   BEFORE DELETE
-- =============================================================================

CREATE OR REPLACE FUNCTION modulo9.trg_fn_dispositivo_no_delete_con_historial()
RETURNS TRIGGER AS $$
DECLARE
    ...
    v_sensores        INTEGER;
BEGIN
    SELECT COUNT(*) INTO v_calibraciones
    FROM modulo9.calibraciones
    WHERE id_dispositivo_iot = OLD.id_dispositivo_iot;

    SELECT COUNT(*) INTO v_configuraciones
    ...
    IF v_calibraciones > 0 OR v_configuraciones > 0 OR v_sensores > 0 THEN
        RAISE EXCEPTION 'DEVICE_HAS_HISTORY: El dispositivo con serial "%" tiene datos históricos asociados (% calibración(es), % configuración(es), % sensor(es)). Use la desactivación lógica (es_activo = false).',
            OLD.serial, v_calibraciones, v_configuraciones, v_sensores
            USING ERRCODE = 'P0129';
    END IF;

    RAISE EXCEPTION 'NO_PHYSICAL_DELETE: Los dispositivos IoT no pueden eliminarse físicamente. Use la desactivación lógica (es_activo = false). Serial: "%".',
        OLD.serial
        USING ERRCODE = 'P0129';
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_dispositivo_no_delete_con_historial();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_sensor_asociacion_unica_activa()
RETURNS TRIGGER AS $$
DECLARE
    v_count      INTEGER;
    v_nombre_area VARCHAR(50);
BEGIN
    SELECT COUNT(*), MAX(i.nombre)
    INTO v_count, v_nombre_area
    FROM modulo9.sensores_areas_asociadas saa
    JOIN modulo9.infraestructuras i ON i.id_infraestructura = saa.id_infraestructura
    WHERE saa.id_sensor    = NEW.id_sensor
      AND saa.tiene_estado = TRUE;

    IF v_count > 0 THEN
        RAISE EXCEPTION 'SENSOR_ALREADY_ASSIGNED: El sensor ID % ya está activo en el área "%". Finalice la asociación actual antes de reasignarlo.',
            NEW.id_sensor, v_nombre_area
            USING ERRCODE = 'P0130';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_sensor_asociacion_unica_activa();
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_calibracion_dispositivo_activo();


-- =============================================================================
-- TRG-M9-26 — Unicidad de nombre de plantilla y versionado incremental
-- Tabla:    modulo9.plantillas
-- Evento:   BEFORE INSERT
-- =============================================================================

CREATE OR REPLACE FUNCTION modulo9.trg_fn_plantilla_version_incremental()
RETURNS TRIGGER AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_plantilla_version_incremental();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_plantilla_inmutable()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'DELETE' THEN
        RAISE EXCEPTION 'IMMUTABLE_TEMPLATE: La plantilla "%" (v%) es inmutable y no puede eliminarse. Las plantillas son registros permanentes.',
            OLD.template_name, OLD.version
            USING ERRCODE = 'P0134';
    END IF;

    IF TG_OP = 'UPDATE' THEN
        RAISE EXCEPTION 'IMMUTABLE_TEMPLATE: La plantilla "%" (v%) es inmutable. Para actualizar, genere una nueva versión mediante INSERT.',
            OLD.template_name, OLD.version
            USING ERRCODE = 'P0134';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_plantilla_inmutable();
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_identidad_visual_colores_validos();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_tema_visual_valor_valido()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.theme_mode NOT IN (1, 2, 3) THEN
        RAISE EXCEPTION 'INVALID_THEME: El tema visual "%" no es válido. Los valores permitidos son: 1 (Claro), 2 (Oscuro), 3 (Automático/Sistema).',
            NEW.theme_mode
            USING ERRCODE = 'P0136';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_tema_visual_valor_valido();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_configuracion_remota_tiempos_validos()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_configuracion_remota_tiempos_validos();
...
CREATE OR REPLACE FUNCTION modulo9.trg_fn_configuracion_remota_sin_pendiente()
RETURNS TRIGGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE EXCEPTION 'PENDING_CONFIG_EXISTS: Ya existe una configuración PENDIENTE para el dispositivo ID %. Espere a que se aplique o cancélela antes de enviar una nueva.',
            NEW.id_dispositivo_iot
            USING ERRCODE = 'P0139';
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION modulo9.trg_fn_configuracion_remota_sin_pendiente();
